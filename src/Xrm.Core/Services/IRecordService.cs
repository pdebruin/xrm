...
{
    Task<RecordPage> GetAllAsync(Guid entityId, int page = 1, int pageSize = 25, string? sortField = null, string sortDir = "asc", string? filter = null, List<ViewFilter>? viewFilters = null, bool? isActive = true);
    ...
    Task<bool> ReactivateAsync(Guid entityId, Guid id);
